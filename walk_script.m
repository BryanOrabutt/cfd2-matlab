...
% Take out the mean and comvert to ps
	...
	if(run_num == 1)
		typical = tpd_no_retriggers;
        typical_tpd = 1e9 .* submatrix(:,5);
	end
...
%     title("Walk Characteristics for CFD Tau_r=20ns", "fontsize", 16) ;
% 
...
%    print(h_summary, '-dpdf', '-color', str) ;
	...
	lerr = energy_avg-energy_min;
    lerr = lerr.*1000;
	uerr = energy_max-energy_avg;
    uerr = uerr.*1000;
    
    energy_avg = energy_avg - mean(energy_avg);

	amplitudes = sortrows(abs(amplitudes));
    errorbar(amplitudes(:,1)', energy_avg.*1000, energy_avg-energy_min, energy_max-energy_avg, 'r.', 'MarkerSize', ...
        80, 'LineWidth', 4, 'CapSize', 20) ;
	%set(eb, 'linestyle', 'none');
	%set(eb, 'marker', '.');
	%set(eb, 'MarkerSize', 25);
	%set(eb, 'LineWidth', 2);
    ...
    set(gca, 'XTick', [25, 100, 200, 500, 1000, 1500, 1600]);
    axis([0 1600 -200 300]);
    set(gca, 'FontSize', 34);
    
    xlabel('Voltage (mV)', "fontsize", 44, 'FontWeight', 'Bold') ;
    ylabel('Delay with respect to Average (ps)', "fontsize", 44, 'FontWeight', 'Bold') ;
    title("Delay vs Energy", "fontsize", 44) ;
%     axis([-50 max(abs(amplitudes(:,1)))*1.15  ... 
%         min(abs(energy_avg-energy_min).*1000)*0.85 ... 
%         max(abs(energy_max-energy_avg).*1000)*1.15]) ;
	hold off;
    %warning("off") ;
